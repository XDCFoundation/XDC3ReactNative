...
const ApprovalAll = (url,token_address,owneraddress,spenderAddress) => {
    // HTTPProvider:
    let httpProvider = new ethers.providers.WebSocketProvider(url);

    let contract = new ethers.Contract(token_address, xrc721_abi, httpProvider);

    let approved = contract.isApprovedForAll(owneraddress,spenderAddress).then(result => { return result })
    return approved;
}
